...
# ggsave(
#   "docs/Publications/07-Forest microclimate/Submission Ecological Informatics/Reviews V1/diagnostics/response_distributions.png",
#   response_density_plot,
#   width = 14,
#   height = 10,
#   dpi = 300
...
fit_microclimate_model <- function(resp, data) {
    ...
    if(str_detect(resp,"temperature")){
        ...
        # Gaussian LMM with continuous-time AR(1)
        mod_ar1 <- nlme::lme(
                fixed = formula_lme,
                random = ~ 1 | site_code,
                correlation = corCAR1(
        ...
        # AR(1) requires an ordered factor with equally spaced time levels
        data_gamma$time_factor <- factor(
            data_gamma$time_days,
            levels = sort(unique(data$time_days)),
            ordered = TRUE
        )

        formula_gamma_AR1 <- as.formula(
            paste0(
            resp,
            " ~ energy_tot * (",
            paste(structural_predictors, collapse = " + "),
            ")",
            "+ (1 | site_code) + ar1(time_factor + 0 | site_code)"
            )
        )
        ...
        # Gamma LMM with continuous-time AR(1)
        mod_ar1 <- glmmTMB(
            formula = formula_gamma_AR1,
            family = Gamma(link = "log"),
            data = data_gamma,
            na.action = na.omit
        )
        
        # Gamma LMM without continuous-time AR(1)
        mod_ind <- glmmTMB(
  ...
  fit_tab <- tibble(
    ...
    AIC_AR1 = AIC(mod_ar1),
    BIC_AR1 = BIC(mod_ar1),
    logLik_AR1 = as.numeric(logLik(mod_ar1)),
  ...
  temporal_comparison <- if (!is.null(mod_ind)) {
    tibble(
      ...
      AIC_AR1 = AIC(mod_ar1),
      delta_AIC_AR1 = AIC(mod_ind) - AIC(mod_ar1),
...
dir.create("docs/Publications/07-Forest microclimate/Submission Ecological Informatics/Reviews V1/diagnostics", showWarnings = FALSE)

for (resp in names(diagnostic_plots)) {
  ggsave(
    filename = file.path(
      "docs/Publications/07-Forest microclimate/Submission Ecological Informatics/Reviews V1/diagnostics",
...
## Temporal autocorelation diagnostic
temporal_results <- purrr::map_dfr(
  model_results,
  "temporal_comparison"
)
...
plot_residual_acf <- function(result) {
...
acf_plots <- lapply(model_results, plot_residual_acf)

for (resp in names(acf_plots)) {
  ggsave(
    filename = file.path(
      "docs/Publications/07-Forest microclimate/Submission Ecological Informatics/Reviews V1/diagnostics",
      paste0(resp, "_residual_ACF.png")
    ),
    plot = acf_plots[[resp]],
    width = 7,
    height = 5,
